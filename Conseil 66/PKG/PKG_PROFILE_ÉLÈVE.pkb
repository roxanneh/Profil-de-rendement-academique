...
function get_pgc_address_home (a_person_id varchar2, a_effdate date) return varchar2 is
a_address_line varchar2(200);
...
O_PEI VARCHAR2(3) := NULL;
O_COUNT number;
BEGIN
SELECT count(*) INTO O_count FROM FS_SE_STUDENT_SETTINGS WHERE
...
O_ALF VARCHAR2(3) := NULL;
O_COUNT number :=0 ;
BEGIN

SELECT count(*) into O_COUNT FROM fs_student_fsl_programs WHERE
...
select MAX(100-(ROUND(SUM(eac.e)*100/(t.no_mots), 2))) into PERCENTAGE
...
FUNCTION GET_YTD_ABS(PI_PERSON_ID VARCHAR2, PI_SCHOOL_YEAR VARCHAR2, PI_DATE date, PI_GRADE varchar2) RETURN VARCHAR2 as
v_abs number :=0;
begin
if PI_GRADE >= 09 then 
...
ELSE 
    select FULL_NAME_F INTO STATUT
    from FS_NATIVE_IDENTIFICATION, persons 
    where person_id = A_PID
    and native_flag = native_id;
END IF;
...
IF (ROW_COUNT > 0) THEN 
    SELECT language_name_f INTO LANG
    FROM person_languages pl, languages l 
    where spoken_at_home_flag = 'x'
